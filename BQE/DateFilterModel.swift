...
public enum DateFilter: String, CaseIterable, Identifiable {
    ...
    public var section: FilterSection {
        ...
        case .today, .yesterday:
            return .specific
        case .thisWeek, .thisMonth, .thisYear, .thisQuarter, .thisFiscalQuarter, .thisFiscalYear, .thisBiWeekly, .thisSemiMonthly:
            return .current
        case .lastWeek, .lastMonth, .lastYear, .lastQuarter, .lastFiscalQuarter, .lastFiscalYear, .lastBiWeekly, .lastSemiMonthly:
            return .past
        case .nextWeek, .nextMonth, .nextQuarter, .nextFiscalYear, .next2Weeks, .next12Weeks, .next12Months, .next12Quarters:
            return .future
        case .thisWeekToDate, .thisMonthToDate, .thisQuarterToDate, .thisYearToDate, .thisFiscalYearToDate:
            return .toDate
        case .asOfToday, .asOfLastMonth, .asOfLastYear:
            return .relative
        case .last7Days, .last30Days, .last60Days, .last90Days, .last180Days, .last365Days, .priorYear, .priorQuarter, .priorMonth:
            return .duration
        }
    }
...
public class DateFilterViewModel: ObservableObject {
    ...
    @Published public var selectedFilter: DateFilter = .thisWeek
        ...
        case .thisWeek:
            ...
            if let startOfWeek = calendar.date(byAdding: .day, value: -daysToSubtract, to: today) {
                ...
                if let endOfWeek = calendar.date(byAdding: .day, value: 6, to: fromDate) {
                    toDate = calendar.date(bySettingHour: 23, minute: 59, second: 59, of: endOfWeek) ?? endOfWeek
                }
            }
        case .lastMonth:
                ...
                if let startOfMonth = calendar.date(from: components) {
                    ...
                    if let nextMonth = calendar.date(byAdding: .month, value: 1, to: startOfMonth),
                       let endOfMonth = calendar.date(byAdding: .day, value: -1, to: nextMonth) {
                        toDate = calendar.date(bySettingHour: 23, minute: 59, second: 59, of: endOfMonth) ?? endOfMonth
                    }
                }
            }
        default:
