...
struct CardStackView: View {
  ...
  @State private var selectedEntryType: EntryType = .time
        ...
        HStack {
          ...
          .disabled(
            stackViewModel.cards.isEmpty
              || (stackViewModel.cards.first.map {
                stackViewModel.getCardState($0.id).action == .skipped
              }
                ?? false)
          )
  ...
  var body: some View {
    HStack(spacing: 24) {
      Menu {
        ...
        HStack(spacing: 8) {
          FAText(iconName: selectedEntryType.icon, size: 12, style: .light)
